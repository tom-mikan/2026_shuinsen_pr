...
# 7. 仮説：「みらい票の90%は参政党の票だった」を検定する {.tabset}

<div class="callout-box">
**仮説の設定**

もしチームみらいの票の90%が本来は参政党に入るべき票だったとしたら、
「参政党の真の得票数」はどうなるか？
そしてその数値は **統計的に説明できるか**？

- **帰無仮説H₀**：参政党の本来の得票率は世論調査支持率3.4%と整合する（自然な範囲）
- **対立仮説H₁**：90%移転後の参政党票数は世論調査から見て統計的に異常
</div>
...
<div class="callout-result">
90%を移転すると、参政党の東京得票率は `r sprintf("%.1f%%", sansei_rate_actual)` → **`r sprintf("%.1f%%", sansei_rate_hypo)`** になります。
世論調査支持率3.4%に対する倍率は **`r sprintf("× %.1f", sansei_rate_hypo/3.4)`** 倍です（実際の参政党は `r sprintf("× %.1f", sansei_rate_actual/3.4)` 倍）。
</div>

## 7-2. 回帰モデルによる妥当性検定

既存の「世論調査支持率 → 東京得票率」の対数回帰モデル（チームみらい除く8政党で構築）に、
仮定後の参政党得票率を当てはめて **予測区間から外れるか** を確認します。

```{r transfer_regression_test, fig.height=6}
# 仮定後の参政党平均得票率（加重平均）
...
## 7-3. 市区町村別の分布：移転前後の参政党得票率

```{r transfer_city_plot, fig.height=9}
...
## 7-4. 結論：90%移転仮説の統計的判定

```{r transfer_conclusion}
pct_above_pi <- mean(df_transfer$参政率_仮定 > pi_upper, na.rm = TRUE) * 100
...
「チームみらいの得票の90%が参政党に流れた」と仮定した場合、
参政党の東京得票率は **`r sprintf("%.1f%%", sansei_hypo_mean)`** となります。

この値は世論調査支持率3.4%に対して **`r sprintf("× %.1f倍", sansei_hypo_mean/3.4)`** であり、
回帰モデルが予測する95%区間（`r sprintf("%.1f%%〜%.1f%%", pi_lower, pi_upper)`）と比較すると

- z スコア = **`r sprintf("%.2f", z_hypo)` σ**
- 片側 p 値 = **`r sprintf("%.4f", p_hypo_upper)`**

`r if(p_hypo_upper < 0.05) "**90%移転後の参政党票数も統計的に異常** です。移転先の参政党にとっても「その規模の票は来ないはず」という別の異常が生じるため、単純な参政党への票移転では矛盾が解消しません。" else "90%移転後の参政党票数は統計的に説明可能な範囲に収まります。ただし、これは「参政党の規模として不自然ではない」という意味に過ぎず、実際に移転があった証拠にはなりません。"`
</div>

---

# 8. 拡張仮説：みらい票90%を「参政・保守・れいわ・減税」に按分した場合 {.tabset}
